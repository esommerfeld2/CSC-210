...
public class CardGame extends JComponent {
	...
	pile[4] = new CardPile(2,402);
        ...
        //pile[3].addFirst(pile[0].removeLast());
        //pile[3].addFirst(pile[0].removeLast());
        //pile[3].addLast(pile[0].removeFirst());
        ...
        //pile[1].addAll(pile[0].split(null));
        ...
        //pile[1].insertBefore(pile[2].removeFirst(), pile[1].getFirst());
        //pile[1].insertAfter(pile[2].removeFirst(), pile[1].getFirst());
        //pile[0].addAll(pile[2].split(pile[2].get(5)));
        //pile[1].insertBefore(pile[0], pile[1].getFirst());
        //pile[1].insertAfter(pile[2], pile[1].getLast());
    ...
    private class Responder implements MouseListener, MouseMotionListener {
        ...
        public void mouseClicked(MouseEvent e) {
            if (e.getClickCount() == 2) {
                ...
                int y = e.getY();
                for (int i = 0; i < pile.length; i++) {
                    Card clickedCard = pile[i].locateCard(x, y);
                    if (clickedCard != null) {
                        ListIterator<Card> position = pile[i].listIterator();
                        while(position.hasNext()){
                            Card place = position.next();
                            if(place == clickedCard){
                                place.flipCard();
                                while(position.hasNext()){
	    ...
	    // What happens here when the mouse is pressed?
            ...
            int y = e.getY();
            for (int i = 0; i < pile.length; i++) {
                CardPile clickedPile = locatePile(x,y);
                Card clickedCard = pile[i].locateCard(x, y);
                if (clickedCard != null) {
                    cardUnderMouse = clickedCard;
                }
                if(clickedPile != null){
                    pileUnderMouse = clickedPile;
                }
            }
        }
        ...
        public void mouseReleased(MouseEvent e) {
            if (movingPile != null) {
                ...
                for (int i = 0; i < pile.length; i++) {
                    ...
                    Card clickedCard = pile[i].locateCard(x, y);
                    if (clickedCard != null) {
                        clickedPile.insertAfter(movingPile, clickedCard);
                        movingPile = null;
                        cardFound = true;
                        break;
                    }
